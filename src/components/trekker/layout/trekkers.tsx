...
/**
 * TrekkerTopbar — PLACEHOLDER
 * TODO: Replace with Sandesh's official TrekkerTopbar when ready
 */
    ...
    <header className="sticky top-0 z-50 border-b border-neutral-200 bg-white">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          {NAV_LINKS.map((link) => {
            ...
            return (
              ...
              >
                {link.label}
                {showBadge && (
                  <span className="inline-flex h-4 min-w-[16px] items-center justify-center rounded-full bg-danger-500 px-1.5 text-[10px] font-semibold text-white">
                    {unreadCount}
                  </span>
                )}
              </Link>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={logout}
            className="flex items-center gap-1.5 rounded-lg border border-neutral-300 px-3 py-1.5 text-sm font-medium text-neutral-700 transition hover:bg-neutral-50"
            aria-label="Logout"
          >
