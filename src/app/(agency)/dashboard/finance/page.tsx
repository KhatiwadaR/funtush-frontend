...
export default function AgencyFinancePage() {
  return (
    <div className="space-y-4">
      <div><div className="flex items-center gap-2 text-sm text-neutral-500"><Link href="/dashboard">Dashboard</Link><span>/</span><span className="font-semibold text-neutral-900">Finance</span></div><h1 className="mt-2 text-2xl font-semibold text-neutral-900">Finance Overview</h1><p className="mt-1 text-sm text-neutral-600">Track income, expenses, profit, and outstanding invoices.</p></div>
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><AnalyticsSummaryCard label="Total Income" value={`Rs. ${totalIncome.toLocaleString()}`} tone="success" icon={ArrowUpToLine} /><AnalyticsSummaryCard label="Total Expenses" value={`Rs. ${totalExpenses.toLocaleString()}`} tone="danger" icon={ArrowDownToLine} /><AnalyticsSummaryCard label="Net Profit" value={`Rs. ${netProfit.toLocaleString()}`} tone="primary" icon={Wallet} /><AnalyticsSummaryCard label="Outstanding Invoices" value={`Rs. ${ouststandingInvoice.toLocaleString()}`} tone="warning" icon={Receipt} /></div>
